...
static int print_number_ul(char *dest, unsigned long long value, format_specifier_t *format)
{
    ...
    auto to_be_minus = MAX(len, format->precision);
    auto paddspace = MAX(format->fillWidth - to_be_minus, 0);
    paddspace = MAX(paddspace, 0);
    auto ret_val = max_width;
    auto printed = 0;
    ...
    while (*ptr != '\0')
    {
        ...
        else
        {
            ...
            while (strchr(_flags_string, *ptr))
            {
                char f = *ptr;
                ptr++;
                if (f == '-') fmt_spcf.flags |= FMTS_FLAGS_LEFTJUSTIFIED;
                if (f == '+') fmt_spcf.flags |= FMTS_FLAGS_FORCESIGN;
                if (f == ' ') fmt_spcf.flags |= FMTS_FLAGS_FILLSPACE;
                if (f == '#') fmt_spcf.flags |= FMTS_FLAGS_LEADSIGN;
                if (f == '0') fmt_spcf.flags |= FMTS_FLAGS_PADZEROES;
            }
