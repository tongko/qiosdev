...
static EFI_STATUS get_vol_dev_path(EFI_DEVICE_PATH_PROTOCOL **out_dp) {
	...
	*out_dp = NULL;
	...
	EFI_STATUS status =
			BS->OpenProtocol(_himage, &_loaded_img_guid, (VOID **)&lip, _himage, NULL,
	...
	status = BS->OpenProtocol(lip->DeviceHandle, &_dev_path_guid, (VOID **)out_dp,
														_himage, NULL, EFI_OPEN_PROTOCOL_GET_PROTOCOL);
	if (EFI_ERROR(status)) {
...
EFI_STATUS extfs_init(const CHAR16 *fname) {
	...
	status =
			BS->LocateHandleBuffer(ByProtocol, &_dev_path_guid, NULL, &count, &hbuf);
	if (EFI_ERROR(status)) {
		BS->FreePool(drv_dp);
